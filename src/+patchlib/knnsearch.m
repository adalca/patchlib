function [patches, pDst, pIdx, pRefIdxs, srcgridsize, refgridsize] = ...
        knnsearch(srcvol, refvols, patchSize, varargin)
% KNNSEARCH k-NN search of patches in source library given a set of reference libraries
%     patches = knnsearch(srclib, reflibs, patchSize) k-NN search of patches in source library
%     srclib given a set of reference libraries refs. refs can be a library or a cell of libraries,
%     of the same dimentionality as srclib. patchSize is the size of the patches. patches is a [M x
%     V x K] array, with M being the number of patches, V the number of voxels in a patch, and K
%     from kNN.
% 
%     patches = knnsearch(srclib, reflibs, patchSize, srcPatchOverlap) allows the specification of a
%     patchOverlap amount or kind for srclib. See patchlib.overlapkind for more information. Default
%     is the default used in patchlib.vol2lib.
% 
%     patches = knnsearch(srclib, reflibs, patchSize, srcPatchOverlap, refsPatchOverlap) allows the
%     specification of a patchOverlap amount or kind for reflibs as well.
% 
%     patches = knnsearch(..., Param, Value) allows for parameter/value pairs:
