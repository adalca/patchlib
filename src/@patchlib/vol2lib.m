...
%   library. vol can be any dimensions (tested for 2, 3); patchSize is the size of the patch
%   (nDims x 1 vector). The default assumption is that the patches overlap by (patchSize - 1)
%   (i.e. 'sliding' patches). See below for options on changing this. The size of the library is
%   then the number of patches that fit in the image x number of voxels in the patch.
%
...
%   libfile.library will load the library into memory. Since you probably had memory constraints,
%   you may want to load the library in parts, just as libfile.library(1, :), etc.
%
%   Current Algorithm:
...
% Contact: {adalca,klbouman}@csail.mit.edu
    ...
    % inputs
    [patchOverlap, dofiledrop, dropfile] = parseInputs(varargin{:});
    nDims = ndims(vol);
    volSize = size(vol);
    
    % get the index and subscript of the initial grid
    [initidx, cropVolSize, gridSize] = patchlib.grid(volSize, patchSize, patchOverlap{:}); 
...
function filelib(vol, patchSize, cropVolSize, initsub, shift, dropfile)

    nGridVoxels = numel(initsub{1});
    rows = ceil(numel(vol) ./ prod(patchSize));
    dropfile.library(nGridVoxels, 1:prod(patchSize)) = 0; % pre-allocation inside matlab file
    for i = 1:rows:nGridVoxels
        range = i:min(i+rows-1, nGridVoxels);
        tmpsub = cellfunc(@(x) x(range), initsub);
        library = memlib(vol, patchSize, cropVolSize, tmpsub, shift);
        
        % todo specify cols?
        dropfile.library(range, :) = library;
    end
end
...
function [patchOverlap, dofiledrop, dropfile] = parseInputs(varargin)
    ...
    if nargin == 2 || nargin == 3
        assert(strcmp(varargin{end-1}, 'savefile'));
        dofiledrop = true;
        dropfile = matfile(varargin{end}, 'Writable', true);
    end
end
